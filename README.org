...
+ ~sudo nrb switch~ to rebuild with the ~priv~ overlay override applied. Pass ~-p <path>~
  to build against a local ~priv~ checkout, ~-f <dir>~ for a flake other than
  ~$HOME/nixos-config~; any other arguments go through to ~nixos-rebuild~
